...
public class ClientService extends AbstractNodeMain {
  ...
  public void onStart(final ConnectedNode connectedNode) {
    ...
    sendMessage("on_Physical Top Bottom 0 0 100");
    sleep(3000);
    sendMessage("near Hand Object1 Object2 0 0 100");
    sleep(3000);
    sendMessage("aboveOf Top Bottom 0 0 100");
    sleep(3000);
    sendMessage("belowOf Bottom Top 0 0 100");
    sleep(3000);
    sendMessage("inFrontOf Object1 Object2 0 0 100");
    sleep(3000);
    sendMessage("behindOf Object1 Object2 0 0 100");
    sleep(3000);
    // sendMessage("objectSize http://www.arbi.com/ontologies/arbi.owl#milk P 0 0
    // 100");
    sleep(1000);
    ...
    // sleep(1000);
    // sendMessage("currentObjectPose H P 0 0 100");
    // sleep(1000);
      ...
      public void onSuccess(MonitorSimilarServiceResponse response) {
        if (response.getResponse().size() != 0) {
          ...
          for (int i = 0; i < response.getResponse().size(); i++)
            System.out.println(response.getResponse().get(i).getPredicate() + " "
                + response.getResponse().get(i).getParam1() + " " + response.getResponse().get(i).getParam2() + " "
                + response.getResponse().get(i).getParam3() + " " + response.getResponse().get(i).getParam4());
