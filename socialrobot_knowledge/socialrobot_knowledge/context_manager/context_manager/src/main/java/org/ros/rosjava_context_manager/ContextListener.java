...
public class ContextListener extends AbstractNodeMain {
	...
	public void onStart(ConnectedNode connectedNode) {
		...
		Subscriber<sensor_msgs.JointState> robot_joint_subscriber = connectedNode
				.newSubscriber("/joint_state", sensor_msgs.JointState._TYPE);
		...
		*/
			...
			public void onNewMessage(std_msgs.Float32MultiArray message) {
				...
				*/
				...
				if (message.getData()[6] == (float) 10) {
					...
					ID = "hubo_right_hand_1";//"hubo_left_hand_1";//robot_hand";
					hIdInd=0;
				} else if (message.getData()[6] == (float) 12) {
					ID = "hubo_left_hand_1";//"robot_finger_mid";
					hIdInd=1;
				} else {
					bIdInd=49;
					hIdInd=49;
				...
				} else if (ID.equals("hubo_left_hand_1") || ID.equals("hubo_right_hand_1")) {
		...
		robot_joint_subscriber.addMessageListener(new MessageListener<sensor_msgs.JointState>() {
			...
			public void onNewMessage(sensor_msgs.JointState message) {
				...
				if(jointPerceptionCount%(6*message.getName().size())==0){
						for(int k=jointPerceptionInterval;k<jointPerceptionCount-message.getName().size()*6;k++)
							retractTriple("'http://www.arbi.com/ontologies/arbi.owl#jointPerception"+k+"' A"+" B C j");
						jointPerceptionInterval=jointPerceptionCount-message.getName().size()*6;
					//	sleep(5);
					}
