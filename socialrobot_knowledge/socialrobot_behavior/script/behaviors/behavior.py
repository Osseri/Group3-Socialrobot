...
class BehaviorBase(with_metaclass(ABCMeta)):
    ...
    def __init__(self, name, **params):
        ...
        self._state_transition[self.INIT_STATE] = (self.check_requirement, 
                {True:self.READY_STATE, False:self.ERROR_STATE})
        self._state_transition[self.READY_STATE] = (self.prepare_behavior, 
                {True:self.RUNNING_STATE, False:self.ERROR_STATE})
        self._state_transition[self.RUNNING_STATE] = (self.run_behavior, 
                {1:self.DONE_STATE, 0:self.RUNNING_STATE, -1:self.ERROR_STATE})
        self._state_transition[self.DONE_STATE] = (self.finish_behavior, 
                {True:self.DONE_STATE, False:self.ERROR_STATE})
        self._state_transition[self.ERROR_STATE] = (lambda:False, 
                {True:self.ERROR_STATE, False:self.ERROR_STATE})
