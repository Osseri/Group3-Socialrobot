...
class DomainGenerator:    
    ...
    def _write_action_predicates(self, file, pred):        
        ...
        if pred_name == "not" or pred_name == "and" or pred_name == "or":
            ...
            file.write("\t\t(")
            if pred_name == "equal":        #equality conidtion
                pred_name ="="
            file.write(pred_name)
            #file.write(" ")  
    ...
    def _write_predicates(self, file):
        ...
        for pred in self.predicates:
            ...
            params = self.predicates[pred]['parameter']
            for param in params:
                file.write(" ")
                file.write(param['name'])
                file.write(" - ")
    ...
    def _get_group_list(self):        
        #load robot group list from moveit commander
        try:
            moveit_commander.roscpp_initialize(sys.argv)
            robot = moveit_commander.RobotCommander()
            group_list = robot.get_group_names()
        except:
            rospy.logwarn('Cannot load robot model. Launch the robot description')
        # TODO: set hardware list from .srdf file

        # key_arm = ['arm']

        # if any(word in key_arm for word in group_list):
        #     print 'found Arm'
        # for i in group_list:
        #     print i.lower().find('arm')

        group_list = ['Arm','Gripper']
        return group_list
