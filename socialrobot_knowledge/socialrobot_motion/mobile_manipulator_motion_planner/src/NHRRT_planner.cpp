...
bool NH_RRT::StartNHRRT(std::ostream& sout) {
	...
	while (iter < 10000) {
		...
		for (int i = 0;i < obs_num;i++) {
			...
			if (!CheckCollision_base(&Obs[i], q_rand)) {
				collision_++;
			}
		}
		if (collision_ > 0) {
		...
		while (rootreached > 0) {
			...
			vnew v_new_i = Extend_base(v_to_extend, q_rand);

			// check collision for every edge of v_new_i
			for (int i = 0;i < obs_num;i++) {
				collision2_ += Check_edge(&Obs[i], v_new_i);
			}
...
Vector3d NH_RRT::RandomConfig_base() {
	...
	if (cntId > 50) {
		...
		if (goal_bias > 0.9) {
			for (int i = 0; i < R.size(); i++) {
				double jointrange = upper_limit_goal_bias(i) - lower_limit_goal_bias(i);
				double r = ((double)rand() / (double)RAND_MAX)*jointrange;
				R(i) = lower_limit_goal_bias(i) + r;
			}
		}
	}
...
vnew NH_RRT::Extend_base(vnew & v_near, Vector3d &q_rand) {
	...
	Q_new = PositionCTRL_WP(q0, q_rand, 1, 0.1, 0.6);

	v_new.id = cntId;
	v_new.pose = Q_new.back();
	v_new.edgeq = Q_new;
	v_new.parent_id = v_near.id;
	v_new.cost = Compute_cost(Q_new, q_rand) + v_near.cost;
...
bool NH_RRT::CheckCollision_base(Obs_para* Obs, Vector3d q) {
	...
	double robot_max_size = max(base_length, base_width);
