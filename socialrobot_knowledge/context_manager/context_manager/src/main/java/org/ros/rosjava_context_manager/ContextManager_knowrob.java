/*
    ...
    */
     ...
     */
    public class ContextManager_knowrob extends AbstractNodeMain {
      ...
      public void onStart(final ConnectedNode connectedNode) {
        ...
        ContextManager_knowrob.connectedNode = connectedNode;
        params = connectedNode.getParameterTree();  

        ContextManager_knowrob.log = connectedNode.getLog();
      ...
      basePath = new File(ContextManager_knowrob.class.getProtectionDomain().getCodeSource().getLocation().getPath()).getAbsolutePath();
      ...
      knowrobFilePath = new File(basePath + "cmProlog/knowrob_library/social_.owl").getAbsolutePath();
        ...
        try {
	  	...
	  	if(request.getManager().equals("FSD"))FSDLength -=1;
			...
			if(request.getManager().equals("FSD")){
				if(request.getStatus()==100&&request.getPredicate().equals("rdf")){

                                if(resp.getParam1().contains("social_")||resp.getParam3().contains("social_"))continue;
		  
				  // make graph
				  MonitorServiceRequest tmpRequest = tmpMonitorRequestGenerator.newMessage();
				  tmpRequest.setPredicate(resp.getParam2());
				  tmpRequest.setParam1(resp.getParam1());
				  tmpRequest.setParam2(resp.getParam3());
				  tmpRequest.setParam3("");
		...
		if(FSDLength == 0){
			...
			if(request.getStatus()==104){

				saveKnowRobGraph();

			}
			if(request.getStatus()==105){
			log.info("fff");
