...
def influence(ob, target, obstacle, Body_position, d_max, eta, obstacle_r):
    ...
    if vector_zero:
        min_angle = (e[0] - np.rad2deg(rotate_angle) + min(vector_zero)) % 360
        max_angle = (e[0] - np.rad2deg(rotate_angle) + max(vector_zero)) % 360
        robot_angle = (e[0] - np.rad2deg(rotate_angle) + 45) % 360
        center_angle = (min_angle + (abs((min_angle - max_angle + 180) % 360 - 180)) / 2) % 360
        if robot_angle < center_angle:
            approaching_angle = 0 - (abs((0 - center_angle + 180) % 360 - 180)) / 4
        elif robot_angle >= center_angle:
            approaching_angle = 0 + (abs((0 - center_angle + 180) % 360 - 180)) / 4
    else:
        approaching_angle = 0.0
    return [x_min, y_min, np.deg2rad(angle_min)], [x_max, y_max, np.deg2rad(angle_max)], [x_dmax[km[0][1]], y_dmax[km[0][1]], np.deg2rad(beta_e[km[0][1]]), km[0][1]], target_direction, approaching_angle
    # return [x_min, y_min, np.deg2rad(angle_min)], [x_max, y_max, np.deg2rad(angle_max)], [x_dmax[km[0][1]], y_dmax[km[0][1]], np.deg2rad(beta_e[km[0][1]]), km[0][1]], target_direction, low_direction, low_angle, km