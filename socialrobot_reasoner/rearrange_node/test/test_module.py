...
def get_request():
    ...
    req.target = get_env_object_info(
        "obj_gotica", (+4.0000e-01, -1.5003e-02, +8.2750e-01), DEFAULT_QUAT, DEFAULT_SCALE
    )
    # The obstacles on the workspace object
    req.objects = [
        get_env_object_info(
            "obj_red_gotica", (+3.0000e-01, +9.9997e-02, +8.2750e-01), DEFAULT_QUAT, DEFAULT_SCALE
        ),
        get_env_object_info(
            "obj_juice2", (0.3, 0.0, +8.2750e-01), DEFAULT_QUAT, DEFAULT_SCALE
        ),
        # get_env_object_info(
        #     "obj3", (0.75, -0.1, +8.2750e-01), DEFAULT_QUAT, DEFAULT_SCALE
        # ),
    ]
    # The workspace object
    req.workspace = get_env_object_info(
        "table", (+5.5001e-01, +8.8066e-06, +3.6501e-01), DEFAULT_QUAT, (7.0887e-01, 1.1342e+00, 6.9000e-01)
        #"table", (+5.5001e-01, +8.8066e-06, +3.6501e-01), (0,0,0.707,0.707), (1.1342e+00, 7.0887e-01, 6.9000e-01)
    )
    return req
...
def srv_visualize(request, response):
    ...
    req_pub = rospy.Publisher(
        "rearrangement_marker/request_view", MarkerArray, queue_size=10, latch=True
    )
    ...
    resp_pub = rospy.Publisher(
        "rearrangement_marker/response_view", MarkerArray, queue_size=10, latch=True
    )
