...
CheckerboardNode::CheckerboardNode(ros::NodeHandle* nodehandle) : nh_(*nodehandle){
    ...
    pub_pose_ = nh_.advertise<geometry_msgs::PoseStamped> ( (node_name+"/pose").c_str(), 1 );
    // Advert marker publisher
    pub_markers_ = nh_.advertise<marker_msgs::MarkerDetection> ( (node_name+"/markers").c_str(), 10 );
    // Advert fiducial publisher
    pub_fiducials_ = nh_.advertise<marker_msgs::FiducialDetection> ((node_name+"/fiducials").c_str(), 10 );
    // Image publisher
    pub_image_ = it_.advertise((node_name+"/chessboard").c_str(), 1);

    tf_broadcaster_ = std::make_shared<tf::TransformBroadcaster>();

    nh_.param<std::string> ( "frame_id", checkerboard_frame_id_, node_name+"_chessboard" );

    reconfigureServer_ = new dynamic_reconfigure::Server<vision_calibrator::CheckerboardDetectionConfig> ( ros::NodeHandle ( "~" ) );
...
void CheckerboardNode::callbackConfig ( vision_calibrator::CheckerboardDetectionConfig &_config, uint32_t _level ) {
    config_ = _config;
...
void CheckerboardNode::callbackCamera ( const sensor_msgs::ImageConstPtr& image_msg,
    ...
    if ( config_.fast_check ) flags += cv::CALIB_CB_FAST_CHECK;
    ...
    if ( patternfound ) {
        ...
        if ( config_.show_camera_image ) {
            ...
            putText ( image_rgb_, "X", pi1, font, fontScale, CV_RGB ( 255,0,0 ), thickness, cv::LINE_AA );
            ...
            putText ( image_rgb_, "Y", pi2, font, fontScale, CV_RGB ( 0,255,0 ), thickness, cv::LINE_AA );
            ...
            putText ( image_rgb_, "Z", pi3, font, fontScale, CV_RGB ( 0,0,255 ) , thickness, cv::LINE_AA );
    ...
    if ( config_.show_camera_image ) {
        ...
        image_msg_ = cv_bridge::CvImage(std_msgs::Header(), "bgr8", image_rgb_).toImageMsg();
...
int main ( int argc, char** argv ) {
    ...
    ros::NodeHandle nh;
    CheckerboardNode checkerboard_node(&nh);
