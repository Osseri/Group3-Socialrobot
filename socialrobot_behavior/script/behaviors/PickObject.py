...
class PickObjectBehavior(BehaviorBase):
    def __init__(self, name, **params):
        super(PickObjectBehavior, self).__init__(name, **params)
        ...
        self.rot_x = lambda x: quaternion_about_axis(x / 180. * math.pi, (1, 0, 0))
        self.rot_y = lambda x: quaternion_about_axis(x / 180. * math.pi, (0, 1, 0))
        self.rot_z = lambda x: quaternion_about_axis(x / 180. * math.pi, (0, 0, 1))
    ...
    def _call_ros_service(self, requirements):
        ...
        arm_req.targetBody = robot_group
        manipulability_req.targetBody = robot_group
        if robot_group == MotionPlanRequest.LEFT_ARM:
            grasp_req.targetBody = MotionPlanRequest.LEFT_GRIPPER
        elif robot_group == MotionPlanRequest.RIGHT_ARM:
        ...
        objects = requirements.dynamic_object + requirements.static_object
        obstacle_ids = []
        obstacles = []
        for obj in objects:
            obstacle_ids.append(obj.id)
        ...
        grasp_req.targetObject = [target_object.id]
        ...
        #         grasp_p = calc_pose_offset(target_object.bb3d.center, quat, 0.17)
        #         grasp_p.orientation.x = quat[0]
        #         grasp_p.orientation.y = quat[1]
        #         grasp_p.orientation.z = quat[2]
        #         grasp_p.orientation.w = quat[3]
        #         target_object.grasp_point.append(grasp_p)

        # robocarerobot wrist forward direction: [0, 0.707, 0, 0.707]
        sample_deg = [180, 225, 135, 270, 90]
        for i in sample_deg:
            wrist_pose, eef_pose = self.get_grasp_pose(degToRad(i), target_object.bb3d.center, robot_group)
            target_object.grasp_point.append(wrist_pose)
        ...
        for i, grasp_pose in enumerate(target_object.grasp_point):
            # publish sample pose
            self.talker.sendTransform(
                (grasp_pose.position.x, grasp_pose.position.y, grasp_pose.position.z),
                (grasp_pose.orientation.x, grasp_pose.orientation.y, grasp_pose.orientation.z,
                    grasp_pose.orientation.w), rospy.Time.now(), "approach_sample_pose", "base_footprint")
    ...
    def get_grasp_pose(self, rad, target_center, robot_group):
        eef_pose = Pose()
        wrist_pose = Pose()
        ...
        #
        target_pos = target_center.position
        eef_pose.position.x = target_pos.x + (0.17) * math.cos(rad)
        eef_pose.position.y = target_pos.y + (0.17) * math.sin(rad)
        eef_pose.position.z = target_pos.z 

        init_quat = [0, -0.707, 0, 0.707]
        quat = quaternion_multiply(init_quat, quaternion_about_axis(rad, (1, 0, 0)))
        ...
        if robot_group == MotionPlanRequest.RIGHT_ARM:
            quat = quaternion_multiply(np.array([quat[0], quat[1], quat[2], quat[3]]), self.rot_z(180))
            eef_pose.orientation.x = quat[0]
            eef_pose.orientation.y = quat[1]
            eef_pose.orientation.z = quat[2]
            eef_pose.orientation.w = quat[3]
...
if __name__ == "__main__":
    ...
    pb = PickObjectBehavior('pick')

    plan_req = GetMotionRequest()

    # arm type
    plan_req.planner_name = "pickobject"
