...
class OpenHandBehavior(BehaviorBase):
    ''' Gripper Open-Close  '''
    def __init__(self, name, **params):
        super(OpenHandBehavior, self).__init__(name, **params)
        ...
        self.social_robot_gripper = {
            ...
            'left_close': [0.0311, 0.1243, 0.1864],
            'right_open': [-0.34, -0.34, 0.34],
            'right_close': [0.0311, 0.1243, -0.1854]
        }
    ...
    def _call_ros_service(self, requirements):
        '''
        requirements [socialrobot_msgs/Behavior]
        '''
        ...
        if robot_group == MotionPlanRequest.LEFT_GRIPPER or robot_group == MotionPlanRequest.LEFT_ARM:    
            ...
            pos_start.positions = self.openclose_joints.get('right_close') + self.openclose_joints.get('left_close')
            pos_end.positions = self.openclose_joints.get('right_open') + self.openclose_joints.get('left_open') 
