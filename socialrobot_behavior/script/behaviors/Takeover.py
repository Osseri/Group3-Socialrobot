...
class TakeoverBehavior(BehaviorBase):
    ''' Joint Trajectory Following '''
    def __init__(self, name, **params):
        super(TakeoverBehavior, self).__init__(name, **params)
    ...
    def get_motion(self, inputs):
        ...
        if ret == MotionPlanResponse.SUCCESS:
            rospy.loginfo("Takeover planning is done.")
            ...
            rospy.loginfo("Takeover planning is failed.")
    ...
    def _call_ros_service(self, inputs):
        try:
            ...
            if len(inputs.targetObject) == 0:
                rospy.logerr("Takeover target is not decided.")
